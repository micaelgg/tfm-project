...
def get_data():
    ...
    emotion_dic = {'anger': 0, 'disgust': 1, 'fear': 2, 'happiness': 3, 'sadness': 4, 'surprise': 5, 'neutral': 6,
                   'calm': 7, 'boredom': 8}
    emo_number = []
    print("global 1------- " + str(globalvars.nb_classes))
    for emo in globalvars.emotions:
        emo_number.append(emotion_dic[emo])

    print("Loading data and features...")
    db = pickle.load(open(globalvars.dataset + '_db.p', 'rb'))
    f_global = pickle.load(open(globalvars.dataset + '_features_sequence.p', 'rb'))

    y = np.array(db.targets)
    print("Number of samples: " + str(len(y)))
    index_select_emotions = np.isin(y, emo_number)

    y = y[index_select_emotions]
    f_global = f_global[index_select_emotions]

    category_translated = 0
    text_txt = ""
    for j in emo_number:
        y[y == j] = category_translated
        text_txt += str(category_translated) + "-" + str(j) + "\n"
        category_translated += 1
    with open("translated-original-emotion.txt", "w") as file:
        file.write(text_txt)
...
def get_data():
    ...
    x_train, x_test, y_train, y_test = train_test_split(f_global, y, test_size=0.20, random_state=101)
...
if __name__ == '__main__':
    ...
    hora_inicio = time.strftime("%c")
    try:
        """ Crea el modelo y lo optimiza"""
        trials = Trials()
        ...
        print("Hora fin: " + time.strftime("%c"))
    except IOError:
